...
## Responsibilities
1. **Profound Synthesis:** The standard `paper-analyzer` simply extracts facts (e.g., "Yantra uses Prefect"). The `deep-analyzer` connects the dots (e.g., "Yantra uses Prefect *because* imperative orchestration in Python pipeline applications leads to fragile, non-idempotent workflows, thus Prefect provides a declarative, fault-tolerant execution boundary").
...
4. **Formal Complexity Analysis (ESWA Requirement):** When processing the "Proposed Methodology" section, explicitly generate:
   ...
   - A Complexity Comparison Table structurally contrasting the proposed agentic pipeline against standard baselines.
